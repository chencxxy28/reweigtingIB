...
###########################
#Use information_borrowing_gim_binary to run gim on ib
...
gim_parallel<-function(r, i, n){
  ...
  N=n*r
      ...
      V_tilde=ginv(EPH_e)%*%ginv(SSt_e)%*%ginv(EPH_e)/N
      
      gim<-information.borrowing_gim_binary(beta_initial = beta, theta_ext_ini = c(1,1,1,1), 
                                            theta_ext_est = theta_e, v_ext = V_tilde, y_int=y, x_int=xm, used_index = c(1,3,4,5))
      
    ##save(list=ls(all.names=TRUE), file=".RData", envir=.GlobalEnv)
      beest<-gim$beta_est
     save(beest, file=paste(r, i,".RData", sep="_"))
     return(0)
}
...
for(r in ratio)
{
  N<-n*r
    for(i in 1:iteration)
    {
      ...
      beta_gim<-rbind(beta_gim, gim$beta_est)
      theta_gim<-rbind(theta_gim, gim$theta_est)
